...
class _WorkspaceScreenState extends State<WorkspaceScreen> {
  ...
  int _lastTreeRevision = 0;

  @override
  void didChangeDependencies() {
    super.didChangeDependencies();
    final editor = context.read<EditorState>();
    if (_lastTreeRevision != editor.treeRevision) {
      _lastTreeRevision = editor.treeRevision;
      _clearTreeCache();
      if (widget.state.selectedRepo != null) {
        _fetchTree('', force: true);
      }
    }
  }
  ...
  Future<void> _openFile(_TreeNode node) async {
    ...
    try {
      ...
      // 移除所有文件大小限制，统一使用 EditorState 的分页缓冲机制加载
      if (!mounted) return;
      context.read<EditorState>().openFileFromBase64(node.path, raw);
...
class _TreeTileState extends State<_TreeTile> {
  ...
  Widget build(BuildContext context) {
    ...
    const rowH = 28.0;
    final indent = 8.0 + widget.depth * 14.0;
    ...
    final dirColor = isDark ? const Color(0xFF80CBC4) : const Color(0xFF00897B);
    final baseTextColor = scheme.onSurface.withOpacity(isDark ? 0.85 : 0.75);
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: isSelected
              ? scheme.primary.withOpacity(isDark ? 0.15 : 0.10)
              : Colors.transparent,
        ...
        child: InkWell(
          ...
          child: SizedBox(
            ...
            child: Row(children: [
              ...
              if (node.isDir)
                Icon(node.expanded ? Icons.keyboard_arrow_down_rounded : Icons.chevron_right_rounded, size: 16, color: scheme.onSurface.withOpacity(0.4))
              else
                const SizedBox(width: 16),
              const SizedBox(width: 2),
              Icon(
                node.isDir ? (node.expanded ? Icons.folder_open_rounded : Icons.folder_rounded) : _icon(),
                size: 16,
                color: node.isDir ? dirColor : _iconColor(),
              ),
              ...
              Expanded(child: Text(node.name, maxLines: 1, overflow: TextOverflow.ellipsis,
                  style: TextStyle(fontSize: 12.5,
                      ...
                      fontWeight: isDirty || node.isDir ? FontWeight.w600 : FontWeight.w400,
                      color: isSelected
                          ? scheme.primary
                          : baseTextColor))),
              if (isDirty) ...[
